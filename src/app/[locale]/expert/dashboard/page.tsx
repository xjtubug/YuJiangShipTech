      ...
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            {expert?.avatar && (
              <Image src={getImageUrl(expert.avatar)} alt={expert?.name || 'Expert avatar'} width={40} height={40} className="w-10 h-10 rounded-full object-cover" />
            )}
            <div>
          ...
          {products.map(product => (
            <div key={product.id} className="bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-md transition">
              <div className="aspect-video bg-gray-100 relative">
                <Image src={getImageUrl(getProductImage(product))} alt={product.nameZh} width={800} height={450} className="w-full h-full object-cover" />
              </div>
              <div className="p-4">
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {reviewImages.map((img, idx) => (
                    <div key={idx} className="relative w-20 h-20">
                      <Image src={getImageUrl(img)} alt={`product-img-${idx}`} width={400} height={300} className="w-full h-full object-cover rounded-lg" />
                      <button
